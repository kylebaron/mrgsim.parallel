...
# Introduction
mrgsim.parallel provides a parallelized simulation workflows for use with 
...
and therefore this strategy will only speed up problems with some size to them; 
you will see little performance gain and possibly some slowdown by trying to 
parallelize problems that are smaller or run very fast to begin with.
...
gain; that is not the point. I will include some larger examples at the end 
of the vignette that aren't actually run when the vignette builds but the user
can copy and run the code as a first try at parallelization.
...
`future.apply` package for the parallel backend.
