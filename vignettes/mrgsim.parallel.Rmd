...
example, we can use `future.callr::callr` with 2 workers
...
and `future::callr`, which requires new R processes to be started up.
...
In the previous section, we saw how you can simulate in parallel, running all 
the jobs in the background. In that example, we saved output generated on worker 
nodes in `fst` format. We can alternatively save outputs in very efficient file 
formats provided by the Apache Arrow package, providing a powerful way to 
manage large outputs. 

We prefer the parquet format provided by the arrow R package. To take this 
approach, you'll first have to install arrow. 


Once arrow is installed, we can simulate in the background again, this time
saving with `.format = "parquet"`
...
Because we set `.locker`, the simulation outputs can be found in that 
directory
...
We have eight data chunks sitting in parquet-formatted files. To read the data
back into R, we use the `arrow::open_dataset()` function and point to the 
_directory_ where those files are stored

```{r}
ds <- arrow::open_dataset(locker)
```
...
We can read it all back into R in one call
...
Or, we can call some wrangling functions on the data set object before 
calling `collect()`
...
While this specific example is very small and easy to deal with, using this 
arrow / dataset approach is an efficient way to deal with very large simulation
output.
