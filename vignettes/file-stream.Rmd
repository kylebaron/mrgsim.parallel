...
This creates a file stream, which is just a list with on position representing
each replicate we want to do

```{r}
length(x)
```

Each slot contains another list containing information for the `ith` replicate. 
...
The stem of the output file named with the current replicate number `05` and 
the total number of replicates in the set `10`.  The file stem will always 
...
We still have `i` (the replicate number). But not `file` is populated with 
...
```
So when the file stream was created, so was the directory where the files would
...
will be save in `fst` format when the time comes. `fst` a very efficient format
...
To save the `ith` replicate to it's pre-defined file location, we call the 
...
Notice that we passed the `fs` object as the first argument to 
`write_stream()`. This will dispatch to the method that writes `ans` out 
in `fst` format. 

Notice also that we didn't return the data; that's part of the strategy where
we write the data to disk rather than return a potentially massive amount of 
data that could easily swamp our R session. But we did return the `location`
...
or to manipulate the file extension
```{r}
x <- ext_stream(x, "")
x[[4]]$file
```
...
specify a directory that does not exist; if he directory exists, you'll get 
an error. 
...
To prevent the locker from being reset, use `retire_locker()`; this removes
the hidden file designating the directory as a valid locker space and prevents
